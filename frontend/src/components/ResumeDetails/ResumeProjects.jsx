...
export default function ResumeProjects({ projects }) {
  ...
  return (
    <section className='flex flex-col gap-2 bg-[#F9F9F9] p-4 border-l-2 border-primary rounded-r-xl'>
      <h3 className='font-bold text-xl text-primary'>Projects</h3>
      ...
      <div className='space-y-4'>
        {projects.map((proj, i) => (
          ...
          >
            <div className='font-semibold text-primary-900'>{proj.project_title}</div>
            {proj.technologies && (
              <div className='text-gray-400 text-xs font-semibold'>
                <span>{proj.technologies}</span>
              </div>
            )}
            <div className='text-gray-700 text-sm mt-1'>{proj.project_summary}</div>
