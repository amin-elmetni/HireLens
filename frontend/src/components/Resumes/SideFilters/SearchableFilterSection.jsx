...
const INITIAL_BATCH = 20; // Number of items to show instantly
const BATCH_SIZE = 20; // Number of items to add per interval
...
const SearchableFilterSection = ({ title, items, selectedItems, onToggleItem }) => {
  ...
  // Reset visibleCount when collapsed or search changes
  useEffect(() => {
    if (!isExpanded) {
  ...
  }, [visibleCount, isExpanded, filteredItems.length]);
